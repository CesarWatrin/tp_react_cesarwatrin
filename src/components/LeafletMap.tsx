...
const LeafletMap = ({ pubs, addPub, removePub, selectedPubs, keystring }: IProps): JSX.Element => {
        ...
        <SMapContainer>
            ...
            >
                ...
                {pubs.map((pub: IPub, index: number) => {
                    return (
                        <Marker position={[pub.latlng.lat, pub.latlng.lng]} key={pub._id + keystring + index}>
                            <Popup>
                                <PubThumbnail
                                    pub={pub}
                                    addPub={addPub}
